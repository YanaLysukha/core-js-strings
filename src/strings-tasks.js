...
function removeLastOccurrences(str, value) {
  const firstInd = str.lastIndexOf(value);
  if (firstInd < 0) {
    return str;
  }
  const endInd = firstInd + value.length;
  return str.slice(0, firstInd) + str.slice(endInd);
}
...
function countVowels(str) {
  ...
  return arr.filter((char) => 'aeiouyAEIOUY'.includes(char)).length;
}
...
function reverseWords(str) {
  ...
  return currentArr.map((word) => word.split('').reverse().join('')).join(' ');
}
...
function encodeToRot13(str) {
  ...
  const sequence = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const arr = str.split('');
  return arr
    .map((char) => {
